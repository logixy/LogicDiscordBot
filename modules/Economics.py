...
class Economy:
    def __init__(self):
        ...
        self.dbcur.execute(
            "CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, user_id VARCHAR(255), money REAL NOT NULL);"
        )
    ...
    def add_money(self, user, money):
        ...
        if data.fetchone() != None:
            self.dbcur.execute(
                "UPDATE users SET money=money+? WHERE user_id=?", [money, user]
            )
        else:
            self.dbcur.execute(
                "INSERT INTO users (user_id, money) VALUES (?, ?)", [user, money]
            )
        self.db.commit()
...
class Economics(commands.Cog, name="Economics"):
    ...
    async def money_command(
        self, interaction, member: Member = None, ephemeral: bool = True
    ):
        if member == None:
    ...
    async def pay_command(
        ...
        await interaction.response.send_message(
            f"Отправлено **{money}$** пользователю **{member.display_name}**",
            ephemeral=ephemeral,
        )
