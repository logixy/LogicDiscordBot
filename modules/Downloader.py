    ...
    async def dl_worker(self, interaction: Interaction, name: str, video: bool):
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if not check_file:
                    dl_embed.description = "[5/?] File not found webp?!"
                    await interaction.edit_original_response(embed=dl_embed)
                    path = path[: -len(ext)] + "webp"  # slice mp4
                    check_file = os.path.exists(path)
                ...
                if check_file:
                    ...
                    await interaction.edit_original_response(
                        embed=dl_embed, attachments=[File(path)]
                    )
                    os.remove(path)
                else:
                    dl_embed.description = "I... I`m failed.. [File not founnd] " + path
                    await interaction.edit_original_response(embed=dl_embed)
